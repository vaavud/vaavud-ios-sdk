...
public struct RotationProcessor {
    ...
    public init() {
        ...
        if let relVelData = NSUserDefaults.standardUserDefaults().objectForKey("vaavud_relVelsStore") as? NSData {
            if let relVelsStore = NSKeyedUnarchiver.unarchiveObjectWithData(relVelData) as? [[Float]] {
                self.relVelsStore = relVelsStore
                println("Calibration store loaded, completion: \(calibrationCompletionPercentage())")
            }
        }
        
        if let t15Data = NSUserDefaults.standardUserDefaults().objectForKey("vaavud_t15") as? NSData {
            if let t15 = NSKeyedUnarchiver.unarchiveObjectWithData(t15Data) as? [Float] {
                self.t15 = t15
                println("Calibration coefficients loaded: \(self.t15)")
            }
        }
    }
    ...
    mutating func fitAngles(range: Int, centers: [Float], c: FitAngleCapsule) -> [Float] {
        ...
        let angless = centers.map { getAngles(self.anglesN, stride, $0) }
        ...
        if c.merge {
            ...
            angle = map(zip(angless, minIndex(rmsAngleRotation, advanceBy: anglesN))) { $0[$1] }
        }
    ...
    func save() {
        ...
        println("Calibration completion: \(calibrationCompletionPercentage())")
    }
    
    func calibrationCompletionPercentage() -> Float {
        return Float(relVelsStore.filter { $0[0] != 0}.count)/Float(360)
    }
