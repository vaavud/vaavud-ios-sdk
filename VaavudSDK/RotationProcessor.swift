...
public struct RotationProcessor {
    ...
    public init() {
        ...
        indiciesWD = getIndiciesWD(anglesN, length: length)
        ...
        if let relVelData = NSUserDefaults.standardUserDefaults().objectForKey("vaavud_relVelsStore") as? NSData,
            relVelsStore = NSKeyedUnarchiver.unarchiveObjectWithData(relVelData) as? [[Float]] {
                ...
                print("Calibration store loaded, completion: \(calibrationCompletionPercentage())")
        }
    ...
    func save() {
        ...
        print("Calibration completion: \(calibrationCompletionPercentage())")
    }
    
    // fixme: choose one
    func calibrationCompletionPercentage() -> Float {
        
        let n = relVelsStore.reduce(Float()) { $1[0] != 0 ? $0 + 1 : $0 }/360
        let o = Float(relVelsStore.filter { $0[0] != 0 }.count)/Float(360)

        if n != o {
            fatalError("Reduce failed")
        }
        
        return relVelsStore.reduce(Float()) { $1[0] != 0 ? $0 + 1 : $0 }/360
    }
